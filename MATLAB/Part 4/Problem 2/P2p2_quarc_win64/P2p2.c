...
void P2p2_output(void)
{
  ...
  if (rtmIsMajorTimeStep(P2p2_M)) {
    ...
    /* S-Function Block: P2p2/Heli 3D/HIL Read Encoder Timebase (hil_read_encoder_timebase_block) */
    {
      ...
      if (result < 0) {
        ...
        rtb_DeadZonex = P2p2_DW.HILReadEncoderTimebase_Buffer[2];
      }
    }
  ...
  if (rtmIsMajorTimeStep(P2p2_M)) {
    ...
    P2p2_B.ElevationCounttorad = P2p2_P.ElevationCounttorad_Gain * rtb_DeadZonex;
  ...
  if (rtmIsMajorTimeStep(P2p2_M)) {
    ...
    if (rtb_DeadZonex > P2p2_P.DeadZonex_End) {
      rtb_DeadZonex -= P2p2_P.DeadZonex_End;
    } else if (rtb_DeadZonex >= P2p2_P.DeadZonex_Start) {
      rtb_DeadZonex = 0.0;
    } else {
      rtb_DeadZonex -= P2p2_P.DeadZonex_Start;
    }
     ...
     *  Gain: '<S4>/Gain: x'
     */
    P2p2_B.Joystick_gain_x = P2p2_P.Gainx_Gain * rtb_DeadZonex *
   ...
   *  Sum: '<S6>/Sum'
   */
  rtb_Backgain = ((P2p2_B.Gain1 - P2p2_B.DegtoRad_j) * P2p2_P.k_rp -
...
RT_MODEL_P2p2_T *P2p2(void)
{
  ...
  /* External mode info */
  P2p2_M->Sizes.checksums[0] = (2352923045U);
  P2p2_M->Sizes.checksums[1] = (707777784U);
  P2p2_M->Sizes.checksums[2] = (3433206080U);
  P2p2_M->Sizes.checksums[3] = (3525549839U);
  ...
  /* Initialize Sizes */
  P2p2_M->Sizes.numContStates = (3);   /* Number of continuous states */
  ...
  P2p2_M->Sizes.numBlocks = (53);      /* Number of blocks */
  P2p2_M->Sizes.numBlockIO = (14);     /* Number of block outputs */
  P2p2_M->Sizes.numBlockPrms = (150);  /* Sum of parameter "widths" */
  return P2p2_M;
}
